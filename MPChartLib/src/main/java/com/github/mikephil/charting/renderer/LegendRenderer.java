...
public class LegendRenderer extends Renderer {
    ...
    public void computeLegend(ChartData<?> data) {
        ...
        for (int i = 0; i < data.getDataSetCount(); i++) {
            ...
            if (dataSet instanceof IBarDataSet) {
                processBarDataSet((IBarDataSet) dataSet, colors);
            } else if (dataSet instanceof IPieDataSet) {
                processPieDataSet((IPieDataSet) dataSet, colors, entryCount);
            } else if (dataSet instanceof ICandleDataSet) {
                processCandleDataSet((ICandleDataSet) dataSet);
            } else {
                processGenericDataSet(dataSet, colors, entryCount);
            }
        }
    ...
    private void processBarDataSet(IBarDataSet dataSet, List<Integer> colors) {
        String[] stackLabels = dataSet.getStackLabels();

        for (int j = 0; j < colors.size() && j < dataSet.getStackSize(); j++) {
            computedEntries.add(new LegendEntry(
                    stackLabels[j % stackLabels.length],
                    dataSet.getForm(),
                    dataSet.getFormSize(),
        ...
        if (dataSet.getLabel() != null) {
            addDescriptionLabel(dataSet.getLabel());
        }
    }

    private void processPieDataSet(IPieDataSet dataSet, List<Integer> colors, int entryCount) {
        for (int j = 0; j < colors.size() && j < entryCount; j++) {
            computedEntries.add(new LegendEntry(
                    dataSet.getEntryForIndex(j).getLabel(),
                    dataSet.getForm(),
                    dataSet.getFormSize(),
                    dataSet.getFormLineWidth(),
                    dataSet.getFormLineDashEffect(),
    ...
    private void processGenericDataSet(IDataSet dataSet, List<Integer> colors, int entryCount) {
        for (int j = 0; j < colors.size() && j < entryCount; j++) {
            String label = (j == colors.size() - 1 || j == entryCount - 1) ? dataSet.getLabel() : null;
    ...
    private void addDescriptionLabel(String label) {
        computedEntries.add(new LegendEntry(
                label,
